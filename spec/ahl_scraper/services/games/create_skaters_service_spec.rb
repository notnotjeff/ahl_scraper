...
RSpec.describe AhlScraper::Games::CreateSkatersService do
  ...
  it "converts goal data into a team's skater's on ice statlines", :aggregate_failures do
    expect_next_instance_of(AhlScraper::Games::ScoringStatlinesService) { |formatter| expect(formatter).to receive(:call).and_return({ "7117": {} }) }
    expect_next_instance_of(AhlScraper::Games::OnIceStatlinesService) { |formatter| expect(formatter).to receive(:call).and_return({ "7117": {} }) }
    expect_next_instance_of(AhlScraper::Games::PenaltyStatlinesService) { |formatter| expect(formatter).to receive(:call).and_return({ "7117": {} }) }
    expect_next_instance_of(AhlScraper::Games::PenaltyShotStatlinesService) { |formatter| expect(formatter).to receive(:call).and_return({ "7117": {} }) }
    expect_next_instance_of(AhlScraper::Games::ShootoutStatlinesService) { |formatter| expect(formatter).to receive(:call).and_return({ "7117": {} }) }
