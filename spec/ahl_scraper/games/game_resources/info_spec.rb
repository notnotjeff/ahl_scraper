...
RSpec.describe AhlScraper::Games::Info do
  ...
  it "converts raw data into Info record" do
    ...
    expect(info.game_number).to eq(raw_data[:gameNumber])
    ...
    expect(info.started).to eq(raw_data[:started])
    expect(info.final).to eq(raw_data[:final])
    expect(info.public_notes).to eq(raw_data[:publicNotes])
    expect(info.status).to eq(raw_data[:status])
    expect(info.season_id).to eq(raw_data[:seasonId])
